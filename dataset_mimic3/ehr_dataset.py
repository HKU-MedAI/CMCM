...
class EHRdataset(Dataset):
    ...
    def __getitem__(self, index, time_bound=48.0):
        ...
        ys = (
            ...
            if len(ys) > 1
            else np.array(ys, dtype=np.int32)[0]
        )
        return data, ys
...
def get_datasets(discretizer, normalizer, args):
    train_ds = EHRdataset(
        ...
        os.path.join(args.ehr_data_dir, f"{args.task}/train"),
    )
    val_ds = EHRdataset(
        discretizer,
        normalizer,
        f"{args.ehr_data_dir}/{args.task}/val_listfile.csv",
        os.path.join(args.ehr_data_dir, f"{args.task}/train"),
    )
    test_ds = EHRdataset(
        discretizer,
        ...
        os.path.join(args.ehr_data_dir, f"{args.task}/test"),
    )
