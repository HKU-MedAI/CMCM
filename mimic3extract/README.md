...
Following the [codes](https://github.com/YerevaNN/mimic3-benchmarks/) for the publicly available Medical Information Mart for Intensive Care (MIMIC-IV) database ([paper](http://www.nature.com/articles/sdata201635), [website](http://mimic.physionet.org)). 
...
6. The following commands will generate task-specific datasets, which can later be used in models. These commands are independent, if you are going to work only on one benchmark task, you can run only the corresponding command.
       ...
       python -m mimic3benchmark.scripts.create_readmission_30d data/root/ data/readmission/
...
MIMIC-III NOTE Extraction
=========================
Following the [ClinicalNotesICU](https://github.com/kaggarwal/ClinicalNotesICU)

### Text Scripts
Run extract_notes.py file under scripts folder.
Run extract_T0.py file under scripts folder.
